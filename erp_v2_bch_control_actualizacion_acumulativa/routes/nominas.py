...
IMPORT_DEFINITIONS = {
    "trabajadores": {
        ...
        "columns": [
            ...
            ("PARTIDA_CODIGO", True, "Código de partida de esa obra.", "MO-01"),
...
def import_worker_row(data: dict, existing_action: str) -> tuple[str, str]:
    ...
    employee.budget_item_id = item.id
...
def nominas_permission_context() -> tuple[str, str]:
    ...
    if path.startswith("/reportes"):
        module = "reportes"
    elif path.startswith(("/obras", "/partidas", "/api/obras")):
    ...
    if any(term in endpoint for term in ("eliminar", "delete", "baja", "cancelar")):
        action = "eliminar"
...
def protect_csrf():
    """Protección CSRF ligera sin agregar otra dependencia al proyecto."""
    ...
    if (
        ...
        if not expected or not received or not hmac.compare_digest(expected, received):
            abort(400, "La sesión del formulario venció. Recarga la página e intenta nuevamente.")
...
def enforce_nominas_role_scope():
    ...
    if request.method == "GET" and request.endpoint in {
        ...
        if not (
            current_user.tiene_permiso("nomina", "ver")
            or current_user.tiene_permiso("centros_costo", "ver")
        ):
            abort(403)
        return None
    module, action = nominas_permission_context()
...
def item_consumption(item: BudgetItem) -> dict[str, Decimal]:
    ...
    payroll_value = (
        ...
        .join(Payroll)
        .filter(Payroll.estado == "CERRADA", PayrollLine.budget_item_id.in_(ids))
        .scalar()
    )
    imss_value = (
...
def project_consumption(project: Project) -> dict[str, Decimal]:
    ...
    payroll_value = (
        ...
        .join(Payroll)
        .filter(Payroll.estado == "CERRADA", Payroll.project_id == project.id)
        .scalar()
    )
    imss_value = (
        db.session.query(func.coalesce(func.sum(PayrollLine.descuento_imss), 0))
    ...
    if labor_ids:
        labor_value = money(
            ...
            .join(Payroll)
            .filter(Payroll.estado == "CERRADA", PayrollLine.budget_item_id.in_(labor_ids))
            .scalar()
        )
    result = {
        "nomina": money(payroll_value),
...
def payment_channel(method: str | None) -> str:
    """Agrupa cheque con transferencia porque ambos salen de banco."""

    return "EFECTIVO" if (method or "").upper() == "EFECTIVO" else "TRANSFERENCIA"


def weekly_resource_breakdown(
    week_start: date, project_ids: Iterable[int] | None = None
) -> dict:
    """Recurso semanal por concepto y método, sin duplicar préstamos retenidos."""
    ...
    if selected_ids is not None:
        payroll_query = payroll_query.filter(Payroll.project_id.in_(selected_ids or [-1]))
    ...
    for payroll in payrolls:
        ...
        for line in payroll.lines:
            ...
            values["TRANSFERENCIA"]["nomina"] += decimal_value(line.pago_transferencia)

    additional_query = AdditionalPayment.query.filter(
        AdditionalPayment.fecha.between(week_start, week_end)
    )
    office_query = OfficeExpense.query.filter(OfficeExpense.fecha.between(week_start, week_end))
    ...
    if selected_ids is not None:
        ...
        office_query = office_query.filter(OfficeExpense.project_id.in_(selected_ids or [-1]))
    ...
    for payment in additional_query.all():
        values[payment_channel(payment.metodo_pago)]["adicionales"] += decimal_value(
            payment.monto_capturado
        )
    for expense in office_query.all():
        values[payment_channel(expense.metodo_pago)]["adicionales"] += decimal_value(
            expense.monto_capturado
        )
    ...
    availability = {
        ...
        for row in WeeklyResourceAvailability.query.filter_by(semana_inicio=week_start).all()
    }
    for method in ("EFECTIVO", "TRANSFERENCIA"):
        for concept in ("nomina", "adicionales", "subcontratos"):
            values[method][concept] = money(values[method][concept])
        values[method]["requerido"] = money(
            values[method]["nomina"]
            + values[method]["adicionales"]
            + values[method]["subcontratos"]
        )
    ...
    return {
        ...
        "requerido_total": money(
            values["EFECTIVO"]["requerido"] + values["TRANSFERENCIA"]["requerido"]
        ),
        "disponible_total": (
            money(availability.get("EFECTIVO", Decimal("0")) + availability.get("TRANSFERENCIA", Decimal("0")))
...
def loan_remaining(loan: Loan) -> Decimal:
    ...
    return max(Decimal("0"), money(loan.monto) - money(paid))
...
def validate_line_for_close(line: PayrollLine):
    if not line.budget_item or line.budget_item.project_id != line.payroll.project_id:
        raise ValueError(f"{line.nombre_trabajador} no tiene una partida válida de la obra.")
...
def dashboard():
        ...
        capture_progress.append(
            {
                ...
                "progress": progress,
                "pending": not payroll or payroll.estado == "BORRADOR",
            }
        )
    ...
    for project in projects:
        ...
        paid = money(
            sum((p.total_neto for p in records if p.estado == "CERRADA"), Decimal("0"))
        )
        pending = money(
            sum((p.total_neto for p in records if p.estado == "BORRADOR"), Decimal("0"))
        )
        labor_status.append(
            {
                "project": project,
    ...
    resource_summary = weekly_resource_breakdown(
        week_start, None if current_user.is_admin else project_ids
    )
    ...
    return render_template(
        ...
        drafts=sum(1 for p in current_payrolls if p.estado == "BORRADOR"),
        closed=sum(1 for p in current_payrolls if p.estado == "CERRADA"),
    )
...
def user_edit(user_id):
    ...
    projects = Project.query.filter_by(activa=True).order_by(Project.nombre).all()
    if request.method == "POST":
        try:
            ...
            if role not in {"administrador", "capturista", "supervisor", "comprador", "costos"}:
                raise ValueError("Rol inválido.")
            if user.id == current_user.id and role != "administrador":
                raise ValueError("No puedes quitarte tu propio rol de administrador.")
            user.role = role
            selected = [int(value) for value in request.form.getlist("project_ids") if value.isdigit()]
            operational_role = role in {"capturista", "supervisor"}
            user.projects = Project.query.filter(Project.id.in_(selected)).all() if operational_role and selected else []
            user.centro_costo_id = selected[0] if operational_role and selected else None
            ...
            db.session.rollback()
            flash(str(exc), "danger")
    return render_template("users/form.html", user=user, projects=projects)
...
@login_required
def projects_list():
    projects = accessible_projects_query(include_inactive=current_user.is_admin).all()
...
def employees_list():
    ...
    query = Employee.query.options(joinedload(Employee.project), joinedload(Employee.budget_item)).filter(
...
def _employee_form(employee: Employee):
    ...
    if request.method == "POST":
        try:
            ...
            item_id = request.form.get("budget_item_id", type=int)
            if not project_id or project_id not in project_ids:
                raise ValueError("Selecciona una obra/oficina autorizada.")
            item = db.session.get(BudgetItem, item_id) if item_id else None
            if not item or item.project_id != project_id or not item.activa:
            ...
            items = (
                BudgetItem.query.filter_by(project_id=selected_project_id, activa=True)
                .order_by(BudgetItem.codigo)
                .all()
...
def loan_new():
    ...
    if request.method == "POST":
        try:
            ...
            method = selected_payment_method("metodo_entrega")
            loan = Loan(
                employee_id=employee.id,
                fecha_prestamo=parse_date("fecha_prestamo"),
                monto=parse_money("monto", required=True),
                retencion_semanal=parse_money("retencion_semanal", required=True),
                metodo_entrega=method.nombre,
            ...
            if not loan.company_id:
                raise ValueError("Selecciona la empresa que entregó el préstamo.")
            db.session.add(loan)
            db.session.flush()
            updated_drafts = refresh_employee_draft_lines(employee.id)
            audit("CREAR", "PRESTAMO", loan.id, f"{employee.nombre_completo}: {loan.monto}")
            db.session.commit()
            first_week = loan.fecha_prestamo + timedelta(
                days=7 - loan.fecha_prestamo.weekday()
            )
            flash(
                f"Préstamo registrado. La primera semana elegible para retención inicia el "
                f"{first_week.strftime('%d/%m/%Y')}. "
                f"Se actualizaron {updated_drafts} nómina(s) en borrador.",
                "success",
            )
            return redirect(url_for("employee_detail", employee_id=employee.id))
        except ValueError as exc:
...
def loan_cancel(loan_id):
    ...
    if loan.estado == "PAGADO":
        flash("Un préstamo pagado no puede cancelarse.", "danger")
    else:
        loan.estado = "CANCELADO"
        db.session.flush()
        updated_drafts = refresh_employee_draft_lines(loan.employee_id)
        audit("CANCELAR", "PRESTAMO", loan.id)
        db.session.commit()
        flash(
            f"Préstamo cancelado; no generará más retenciones. "
            f"Se actualizaron {updated_drafts} nómina(s) en borrador.",
            "success",
        )
...
def build_line_from_employee(
    ...
    item = previous_line.budget_item if previous_line else employee.budget_item
    if not item or item.project_id != payroll.project_id or not item.activa:
        item = (
            BudgetItem.query.filter_by(project_id=payroll.project_id, activa=True)
            .order_by(BudgetItem.codigo)
            .first()
        )
    if not item:
...
def create_payroll_with_preload(project: Project, week_start: date, creator_id: int) -> Payroll:
    ...
    previous = (
        Payroll.query.filter(
            ...
            Payroll.semana_inicio < week_start,
...
def payroll_new():
    ...
    if request.method == "POST":
        try:
            ...
            if not BudgetItem.query.filter_by(project_id=project.id, activa=True).first():
...
def payroll_detail(payroll_id):
    ...
    items = BudgetItem.query.filter_by(project_id=payroll.project_id, activa=True).order_by(
        BudgetItem.codigo
    ).all()
    companies = Company.query.filter_by(activa=True).order_by(Company.nombre).all()
...
def payroll_save(payroll_id):
    ...
    require_project_access(payroll.project)
    if payroll.estado != "BORRADOR":
        flash("La nómina está cerrada. Un administrador debe reabrirla para editar.", "warning")
        return redirect(url_for("payroll_detail", payroll_id=payroll.id))
    try:
        for line in payroll.lines:
            prefix = f"line_{line.id}_"
            item_id = request.form.get(prefix + "budget_item_id", type=int)
            item = db.session.get(BudgetItem, item_id) if item_id else None
            if not item or item.project_id != payroll.project_id or not item.activa:
                raise ValueError(f"Selecciona una partida válida para {line.nombre_trabajador}.")
            line.budget_item_id = item.id
        ...
        if request.form.get("next_action") == "weekly_report" and current_user.is_admin:
            return redirect(
...
def payroll_add_employee(payroll_id):
    ...
    if request.method == "POST":
        try:
            ...
            line = build_line_from_employee(payroll, employee)
            if not line:
                raise ValueError("No se pudo agregar: revisa que la obra tenga una partida activa.")
            # Si viene de otra obra, se carga a la primera partida activa del destino solo esta semana.
            db.session.flush()
...
def payroll_close(payroll_id):
    ...
    require_project_access(payroll.project)
    if payroll.estado != "BORRADOR":
        flash("La nómina ya estaba cerrada.", "info")
        return redirect(url_for("payroll_detail", payroll_id=payroll.id))
    try:
        ...
        for line, plan in plans:
            for loan, amount in plan:
                db.session.add(LoanPayment(loan_id=loan.id, payroll_line=line, monto=amount))
        ...
        # Al cerrar, queda lista la siguiente semana con los mismos trabajadores activos.
        ...
        if not next_payroll:
            next_payroll = create_payroll_with_preload(payroll.project, next_start, current_user.id)
            audit("PRECARGAR", "NOMINA", next_payroll.id, f"Desde nómina {payroll.id}")
        audit("CERRAR", "NOMINA", payroll.id, f"Neto: {payroll.total_neto}")
        db.session.commit()
        consumption = project_consumption(payroll.project)
        flash(
            f"Nómina cerrada. Costo de mano de obra con IMSS: "
...
def payroll_reopen(payroll_id):
    ...
    for loan_id in loan_ids:
        loan = db.session.get(Loan, loan_id)
        if loan and loan.estado != "CANCELADO" and loan_remaining(loan) > 0:
            loan.estado = "ACTIVO"
    payroll.estado = "BORRADOR"
    ...
    for line in payroll.lines:
        recalculate_line(line)
    audit("REABRIR", "NOMINA", payroll.id, "Retenciones de préstamos revertidas")
    db.session.commit()
...
def build_weekly_closing_report(week_start: date) -> dict:
    ...
    loan_filters = [
        ...
        (Loan.estado == "ACTIVO") & (Loan.fecha_prestamo <= week_end),
    ...
    for loan in loans:
        ...
        projected_paid = money(
            decimal_value(loan.monto) - projected_remaining
        )
        if loan.estado == "CANCELADO":
            week_status = "CANCELADO"
        ...
        project = employee_week_project.get(loan.employee_id) or loan.employee.project
        delivered_this_week = week_start <= loan.fecha_prestamo <= week_end
    ...
    totals["borradores"] = sum(1 for payroll in payrolls if payroll.estado == "BORRADOR")
    totals["cerradas"] = sum(1 for payroll in payrolls if payroll.estado == "CERRADA")
...
def filtered_report_lines():
    ...
    if project_id:
        query = query.filter(Payroll.project_id == project_id)
    if item_id:
        query = query.filter(PayrollLine.budget_item_id == item_id)
    if status in {"BORRADOR", "CERRADA"}:
        query = query.filter(Payroll.estado == status)
    if start:
        try:
...
def reports_index():
    ...
    for line in lines:
        ...
        row["trabajadores"] += 1
    projects = Project.query.order_by(Project.nombre).all()
...
def reports_export():
    ...
    style_excel_sheet(ws, currency_columns=[14, 22, 23, 24, 25, 26, 27, 28, 29, 31, 32, 34, 36])
    ...
    if not selected_project_ids:
        selected_project_ids = [p.id for p in Project.query.all()]
    ...
    loan_query = Loan.query.options(joinedload(Loan.employee), joinedload(Loan.payments)).order_by(
        Loan.fecha_prestamo.desc()
    )
    for loan in loan_query:
        loans_ws.append(
            [
                ...
                loan.employee.project.nombre if loan.employee.project else None,
                loan.fecha_prestamo,
    ...
    style_excel_sheet(loans_ws, currency_columns=range(4, 8))
